...
func _ready() -> void:
	...
	box_mesh.size = Vector3(0.5, 0.6, 1.0)
	_mesh.mesh = box_mesh
	add_child(_mesh)

	# Tail detail: thin box extending behind
	var tail := MeshInstance3D.new()
	var tail_mesh := BoxMesh.new()
	tail_mesh.size = Vector3(0.12, 0.1, 0.7)
	tail.mesh = tail_mesh
	tail.position = Vector3(0, -0.1, 0.7)
	var tail_mat := StandardMaterial3D.new()
	tail_mat.albedo_color = Color(0.15, 0.4, 0.2)
	tail.material_override = tail_mat
	_mesh.add_child(tail)

	_mat_default = StandardMaterial3D.new()
	_mat_default.albedo_color = Color(0.2, 0.5, 0.25)
