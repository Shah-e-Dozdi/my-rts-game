...
func _ready() -> void:
	...
	# Primate body - orange/tan box
	_mesh = MeshInstance3D.new()
	var box_mesh := BoxMesh.new()
	box_mesh.size = Vector3(0.6, 1.1, 0.6)
	_mesh.mesh = box_mesh
	_mesh.position.y = 0.0
	add_child(_mesh)

	# Arm details - two small boxes on sides
	for side in [-1.0, 1.0]:
		var arm := MeshInstance3D.new()
		var arm_mesh := BoxMesh.new()
		arm_mesh.size = Vector3(0.15, 0.4, 0.2)
		arm.mesh = arm_mesh
		arm.position = Vector3(side * 0.35, 0.2, 0.0)
		var arm_mat := StandardMaterial3D.new()
		arm_mat.albedo_color = Color(0.6, 0.38, 0.15)
		arm.material_override = arm_mat
		_mesh.add_child(arm)

	_mat_default = StandardMaterial3D.new()
	_mat_default.albedo_color = Color(0.7, 0.45, 0.2)
...
func _fire_projectile() -> void:
	if _attack_target == null or not is_instance_valid(_attack_target):
		return
	# Thrown projectile - small brown sphere
	...
	mat.emission = Color(0.5, 0.35, 0.15)
