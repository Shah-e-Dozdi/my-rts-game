...
func _ready() -> void:
	...
	box_mesh.size = Vector3(0.3, 0.5, 1.2)
	_mesh.mesh = box_mesh
	add_child(_mesh)

	# Small triangular head detail
	...
	head.position = Vector3(0, 0.1, -0.75)
	var head_mat := StandardMaterial3D.new()
	head_mat.albedo_color = Color(0.4, 0.45, 0.2)
	head.material_override = head_mat
	_mesh.add_child(head)

	_mat_default = StandardMaterial3D.new()
	_mat_default.albedo_color = Color(0.35, 0.4, 0.15)
