...
func _ready() -> void:
	...
	# Wolf body - grey/silver box
	_mesh = MeshInstance3D.new()
	var box_mesh := BoxMesh.new()
	box_mesh.size = Vector3(0.6, 1.0, 0.8)
	_mesh.mesh = box_mesh
	_mesh.position.y = 0.0
	add_child(_mesh)

	# Pointed ear triangles on top
	for side in [-1.0, 1.0]:
		var ear := MeshInstance3D.new()
		var ear_mesh := PrismMesh.new()
		ear_mesh.size = Vector3(0.12, 0.2, 0.12)
		ear.mesh = ear_mesh
		ear.position = Vector3(side * 0.18, 0.6, 0.0)
		var ear_mat := StandardMaterial3D.new()
		ear_mat.albedo_color = Color(0.5, 0.5, 0.55)
		ear.material_override = ear_mat
		_mesh.add_child(ear)

	_mat_default = StandardMaterial3D.new()
	_mat_default.albedo_color = Color(0.55, 0.55, 0.6)
...
func _do_melee_lunge() -> void:
	if _attack_target == null or not is_instance_valid(_attack_target):
		return
	# Lunge animation - tween mesh forward then back
	var orig_pos := _mesh.position
	var tween := get_tree().create_tween()
	tween.tween_property(_mesh, "position", orig_pos + Vector3(0, 0, -0.3), 0.06)
	tween.tween_property(_mesh, "position", orig_pos, 0.12)
